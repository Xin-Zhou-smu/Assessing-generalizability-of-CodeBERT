...
def train(args, train_dataset, model, tokenizer, optimizer):
    ...
    best_acc = 0.0
    model.zero_grad()
    # train_iterator = trange(args.start_epoch, int(args.num_train_epochs), desc="Epoch",  #trange is in tqdm, to show 进度条， epoch的个数就是这里体现的
    ...
    for epoch in range (args.num_train_epochs):
        ...
        for step, batch in enumerate(tqdm(train_dataloader)):
            ...
            if args.fp16:
                ...
                loss.backward()
                torch.nn.utils.clip_grad_norm_(model.parameters(), args.max_grad_norm)  #梯度截断，防止梯度爆炸
        ...
        if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
            ...
            print('\n')
            print('--------------Saving for last-checkpoint!!-------------')
            ...
            if (results['acc'] > best_acc):
                ...
                print('\n')
                print('--------------Saving for best-checkpoint!!-------------')
...
def load_and_cache_examples(args, task, tokenizer, ttype='train'):
    ...
    # if cache files exist, load fearture directly (tokenized Ids by BERT)
    try:
    ...
    # if no cache files, load data from data file; build dataset, tokenization
        ...
        if ttype == 'train':
            examples = processor.get_new_train_examples(args.data_dir, args.train_file)
        elif ttype == 'dev':
            examples = processor.get_new_dev_examples(args.data_dir, args.dev_file)
        elif ttype == 'test':
            examples, instances = processor.get_new_test_examples(args.data_dir, args.test_file)
        ...
        for i in range (2):
            i = i +10
            print(examples[i].text_a)  #<utils.InputExample object at 0x000001B4C6F3C280>
        ...
        #features is a list of InputFeature object
        #print(len(features), type(features[0])) # 28483, <class 'utils.InputFeatures'>
        
        
        # Store generated features (tokenized BERT input_ids, masks, segments)
        if args.local_rank in [-1, 0]:
...
def read_args():
    ...
    parser.add_argument("--per_gpu_train_batch_size", default=32,#default=32, 
                        type=int,
                        help="Batch size per GPU/CPU for training.")
    parser.add_argument("--per_gpu_eval_batch_size", default= 32 #default=32
                        , type=int,
    ...
    parser.add_argument("--train_file", default="triple_train.txt", type=str,                          #### just for test program, use valid dataset
                        help="train file")  
    parser.add_argument("--dev_file", default="triple_dev.txt", type=str,
                        help="dev file")
    parser.add_argument("--test_file", default="batch_0.txt", type=str,
                        help="test file")
    parser.add_argument("--pred_model_dir", default='./models/java_codebert/checkpoint-best', type=str,
                        help='model for prediction')
    parser.add_argument("--test_result_dir", default='./results/python_roberta_stack/test_result.txt', type=str,
...
##-------------------------Set Up-------------------------------##
    ...
    #print('precessor name:',processor)  #<utils.CodesearchProcessor object at 0x00000205A763FEE0>
    args.output_mode = output_modes[args.task_name]
    #print(args.output_mode) # 'classification''
    label_list = processor.get_labels()
    num_labels = len(label_list)  # 2 labels ['0,'1']
...
if __name__ == "__main__":
    ...
    print(len(train_dataset),type(train_dataset)) # 28483 items (each has a lable to say it is matched or not)
    
    print(train_dataset[0][0].size()) # a tuple of tensors-> (input_ids, input_mask, segment_ids, label_id)  --> Tensor.Size (200,1) (200,1) (200,1) (1)
    '''
